...
fn main() -> anyhow::Result<()> {
    ...
    let block_width = img_width.div_ceil(sample_width);
    let block_height = img_height.div_ceil(sample_height);
    ...
    let img_avg = image_blocks.map(|pixels| {
    ...
    let output: String = img_avg.enumerate().fold(|| String::with_capacity(block_width * block_height),
                            |mut acc, (idx, character)| {
